...
import { FEATURED_PRODUCTS } from "@/data/featuredProductsContent";
import { TRENDING_PRODUCTS } from "@/data/trendingNowContent";
import { BEST_SELLER_PRODUCTS } from "@/data/bestSellersContent";
...
export default function HomePage() {
  ...
  const [trending] = useState(TRENDING_PRODUCTS);
  const [bestSellers] = useState(BEST_SELLER_PRODUCTS);
  const [categoriesLoading, setCategoriesLoading] = useState(
    !homeDataCache.categoriesLoaded,
  );
            ...
            <div className="container-page py-8">
              <div className="grid grid-cols-3 sm:grid-cols-6 gap-4">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={i}
                    className="rounded-2xl bg-ink-100 dark:bg-ink-800 animate-pulse aspect-square"
                  />
                ))}
              </div>
            </div>
